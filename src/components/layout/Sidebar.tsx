...
import { Users, QrCode, ScanLine, ShieldCheck, LogOut, Home, Settings, PanelLeftClose, Building } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ user, isOpen, activeTab, onTabChange, onLogout, onToggle }) => {
  ...
  const isSubdomain = !!getTenantFromSubdomain(); // Si es subdominio, no mostramos botón de cambiar tenant
  ...
  const menuItems = [
    ...
    { id: 'generate-qr', label: 'Generar Invitación', icon: QrCode, roles: ['X', 'B'] },
    { id: 'validate-qr', label: 'Recepción / Escáner', icon: ScanLine, roles: ['X', 'A'] },
  ...
  return (
    ...
    >
      <div className="p-6 flex items-center gap-3 border-b border-blue-800">
        ...
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <div className="min-w-0">
            <h1 className="font-bold text-xl tracking-tight truncate">MiVilla</h1>
            <p className="text-xs text-blue-300 truncate" title={tenant?.name}>{tenant?.name}</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-blue-800 space-y-2">
        {/* Switch Tenant Option (Only for Mobile/Direct access) */}
        {!isSubdomain && (
          <button 
            onClick={clearTenant}
            className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-blue-800 hover:bg-blue-700 text-blue-200 rounded-lg transition-all text-xs"
          >
            <Building size={14} />
            <span>Cambiar Condominio</span>
          </button>
        )}
